...
namespace MJHSC.MidtimeEngine.Languages.VisualBasic {
	public class Language : IMidtimeLanguage {

		private EngineEntry LoadScript(string ScriptFilePath) {

			string VB = File.ReadAllText(ScriptFilePath + ".vb");

			VBCodeProvider VBSP = new VBCodeProvider(new Dictionary<string, string> { { "CompilerVersion", "v4.0" } });
			...
			CompilerResults CR = VBSP.CompileAssemblyFromSource(CP, VB);
			if (CR.Errors.Count > 0) {
				foreach (System.CodeDom.Compiler.CompilerError CE in CR.Errors) {
					...
					MessageBox.Show("VisualBasic コンパイルエラー\n\n" +E);
				}
			}
		...
		public MidtimeResponse CanRunScript(string ScriptFilePath) {
			if (File.Exists(ScriptFilePath + ".vb")) {
