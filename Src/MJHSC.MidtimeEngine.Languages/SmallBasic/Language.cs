...
namespace MJHSC.MidtimeEngine.Languages.SmallBasic {
	public class Language : IMidtimeLanguage {

		private EngineEntry LoadScript(string ScriptFilePath) {
			try {
				//File
				MidtimeFile MF = new MidtimeFile(ScriptFilePath + ".exe");
				...
				Type t = dll.GetType("_SmallBasicProgram");
				MethodInfo Method = t.GetMethod("_Main", BindingFlags.Public | BindingFlags.NonPublic | BindingFlags.Static);
		...
		public MidtimeResponse VMMain(string ScriptFilePath) {
			...
			if (EE != null) {
				try {
					...
					MessageBox.Show(string.Format("スクリプト \"{0}.exe\" の実行中にエラーが発生しました。\n\nエラー: {1}\n\n詳細: {2}\n{3}"
