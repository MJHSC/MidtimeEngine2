...
namespace MJHSC.MidtimeEngine.Languages.Lua.HostingV53 {
	public class Language : IMidtimeLanguage {


		internal class LuaDLL {
			//System
			[DllImport(@".\MidtimeEngine\Binaries\Lua.v53.VM.dll", CallingConvention = CallingConvention.Cdecl, CharSet = CharSet.Ansi)]
		...
		internal class LuaAccess53 : ILuaAccess{
			private IntPtr lua_State;

			public LuaAccess53(IntPtr lua_State) {
		...
		public MidtimeResponse CanRunScript(string ScriptFilePath) {
			if (File.Exists(ScriptFilePath + ".lua53")) {
				return MidtimeResponse.OK;
			}
			if (File.Exists(ScriptFilePath + ".lua")) {
				return MidtimeResponse.OK;
			}
		...
		public MidtimeResponse VMMain(string ScriptFilePath) {
			...
			LuaAccess53 LA53 = new LuaAccess53(this.LuaVM);
			LuaLibrary.Register(LA53);


			if (File.Exists(ScriptFilePath + ".lua53")) {
				R = LuaDLL.luaL_loadfilex(LuaVM, ScriptFilePath + ".lua53", IntPtr.Zero);
			} else if (File.Exists(ScriptFilePath + ".lua")) {
				R = LuaDLL.luaL_loadfilex(LuaVM, ScriptFilePath + ".lua", IntPtr.Zero);
			}
			R = LuaDLL.lua_pcallk(LuaVM, 0, 0, 0, IntPtr.Zero);

			if (R != 0) {
				MDebug.WriteFormat("Lua エラー 「{0}」", LA53.GetNextArgAsString(-1));
			}
