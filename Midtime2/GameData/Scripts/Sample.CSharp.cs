...
namespace MidtimeScript{
	class Startup {
		static void Main() {
			//サウンド読み込み
			MMedia BGM = MMedia.CreateSound("UCL-GameBGM"); //BGM読み込み
			BGM.PlayLoop(); //BGM再生。1度だけ開始すればいいため、ループ内に入れてはいけない。

			//画像読み込み
			MImage BG = MImage.CreateImage("GameBG"); //背景画像
			MImage MousePointer = MImage.CreateImage("MousePointer"); //マウス追従テストの画像

			MImage CharStop = MImage.CreateAnime("UCL-UC-Stop", 3, 6, true); //停止中の画像。アニメ
			MImage CharMove = MImage.CreateAnime("UCL-UC-Move", 2, 6, true); //移動中の画像。アニメ
			MImage CharJump = MImage.CreateImage("UCL-UC-Jump"); //ジャンプ中の画像。静止画

			//ヘルプテキスト表示（文字の表示）
			MImage.SetTextColor(0, 0, 0); //黒色
			MImage.SetTextSize(16); //16pt
			MImage HelpText = MImage.CreateText("←→ = 移動、Aボタン (Zキー) = ジャンプ、Bボタン（Xキー） = メニューへ戻る");
			HelpText.SetPosition(160, 505);

			//変数の準備
			int CharX = 20;	//キャラクターの初期位置X
			int CharY = 440; //キャラクターの初期位置Y

			bool LastMoveIsBackward = false; //後ろに移動したときはtrue、前に移動したときはfalse。
			bool NowMoving = false; //キャラが移動中はtrue、停止中はfalse。
